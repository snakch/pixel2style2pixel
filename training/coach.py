...
class Coach:
            ...
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                if self.global_step % self.opts.image_interval == 0 or (
                    ...
                    self.parse_and_log_images(
                        id_logs, x, y, y_hat, title="images/train/faces"
                    )
    ...
    def checkpoint_me(self, loss_dict, is_best):
        save_name = (
            "best_model.pt"
            if is_best
            else "iteration_{}.pt".format(self.global_step)
        )
        save_dict = self.__get_save_dict()
        checkpoint_path = os.path.join(self.checkpoint_dir, save_name)
        torch.save(save_dict, checkpoint_path)
        with open(
            os.path.join(self.checkpoint_dir, "timestamp.txt"), "a"
        ) as f:
            if is_best:
                f.write(
                    "**Best**: Step - {}, Loss - {:.3f} \n{}\n".format(
    ...
    def calc_loss(self, x, y, y_hat, latent, target_latent):
        loss_dict = {}
        loss = 0.0
        id_logs = None
    ...
    def log_images(self, name, im_data, subscript=None, log_latest=False):
        ...
        if subscript:
            ...
            path = os.path.join(
                self.logger.log_dir, name, "{:04d}.jpg".format(step)
            )
